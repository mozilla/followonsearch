...
ADDON="add-on"
WEBEXTENSION="add-on/webextension"
...
# - change 0.1.0-pre to 0.1.0pre for AMO/Firefox compatiblity
...
sed -e "s/\"version\":.*,/\"version\": \"${PACKAGE_VERSION}\",/" \
    ${WEBEXTENSION}/manifest.json > ${WEBEXTENSION}/manifest.json.gen
mv ${WEBEXTENSION}/manifest.json.gen ${WEBEXTENSION}/manifest.json
...
git add ${WEBEXTENSION}/manifest.json
